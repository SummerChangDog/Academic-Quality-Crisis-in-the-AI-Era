...
PAPERS_ROOT = os.path.join(BASE_DIR, "../data/papers")
RESULT_SAVE_PATH = os.path.join(BASE_DIR, "../citation_ai_rate.json")
...
def compare_metadata(parsed: Dict, official_meta: Dict) -> int:
    ...
    print("parsed_authors 列表中的元素：",parsed_authors)
    official_authors = []
    for auth in official_meta.get("author", []):
        if "family" in auth:
            official_authors.append(auth["family"].lower())
    print("official_authors 列表中的元素：", official_authors)
    # 检查作者交集（至少匹配1个核心作者）
...
def process_paper_citations(arxiv_id: str, ref_path: str) -> float:
    ...
    try:
        ...
        if len(citations)>5:
            citations = random.sample(citations, 5)

        citation_levels = []
        for idx, cite in enumerate(citations):
            parsed = parse_citation_text(cite)
            if parsed is None:
                print(f"{arxiv_id} 引用{idx+1}: Failure")
                continue
...
if __name__ == "__main__":
    ...
    # official_meta = search_by_title_author(parsed["title"], parsed["authors"])
    # print(official_meta)
    # print(compare_metadata(parsed, official_meta))
    main()